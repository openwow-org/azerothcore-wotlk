...
Vehicle::~Vehicle()
{
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
        if (itr->second.Passenger.Guid)
        {
            ...
            {
                LOG_FATAL("vehicles", "Vehicle(), unit: {}, entry: {}, typeid: {}, this_entry: {}, this_typeid: {}!", unit->GetName(), unit->GetEntry(), unit->GetObjectTypeID(), _me ? _me->GetEntry() : 0, _me ? _me->GetObjectTypeID() : 0);
                unit->_ExitVehicle();
            }
            else
...
{
    if (_me->GetObjectTypeID() == ID_UNIT)
    {
    ...
    if (GetBase()->GetObjectTypeID() == ID_UNIT)
        sScriptMgr->OnInstall(this);
}

void Vehicle::InstallAllAccessories(bool evading)
{
    if (GetBase()->GetObjectTypeID() == ID_PLAYER || !evading)
...
void Vehicle::Uninstall()
{
    ...
    if (_me && _me->GetObjectTypeID() == ID_UNIT)
    {
...
void Vehicle::Reset(bool evading /*= false*/)
{
    ...
    if (_me->GetObjectTypeID() == ID_PLAYER)
    {
        if (_usableSeatNum)
...
void Vehicle::InstallAccessory(uint32 entry, int8 seatId, bool minion, uint8 type, uint32 summonTime)
{
    ...
    if (Unit* passenger = GetPassenger(seatId))
    {
        ...
        {
            ASSERT(passenger->GetObjectTypeID() == ID_UNIT);
            if (_me->GetObjectTypeID() == ID_UNIT)
            {
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (unit->GetObjectTypeID() == ID_PLAYER && !(seat->second.SeatInfo->m_flagsB & VEHICLE_SEAT_FLAG_B_KEEP_PET))
    ...
    if (_me->GetObjectTypeID() == ID_UNIT
            && unit->GetObjectTypeID() == ID_PLAYER
            && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    {
        ...
        if (!_me->SetCharmedBy(unit, CHARM_TYPE_VEHICLE))
        {
            ...
            LOG_INFO("vehicles", "Crash recovered in Unit::SetCharmedBy(). typeid: {}!", _me->GetObjectTypeID());
            LOG_INFO("vehicles", "Crash recovered in Unit::SetCharmedBy(). Unit {}, typeid: {}, in world: {}, duringremove: {} has wrong CharmType! Charmer {}, typeid: {}, in world: {}, duringremove: {}.", _me->GetName(), _me->GetObjectTypeID(), _me->IsInWorld(), _me->IsDuringRemoveFromWorld(), unit->GetName(), unit->GetObjectTypeID(), unit->IsInWorld(), unit->IsDuringRemoveFromWorld());
            return false;
        }
    }
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    if (seat->second.SeatInfo->CanEnterOrExit() && ++_usableSeatNum)
        _me->SetNpcFlag((_me->GetObjectTypeID() == ID_PLAYER ? UNIT_NPC_FLAG_PLAYER_VEHICLE : UNIT_NPC_FLAG_SPELLCLICK));
    ...
    if (_me->GetObjectTypeID() == ID_UNIT && unit->GetObjectTypeID() == ID_PLAYER && seat->second.SeatInfo->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
    ...
    if (_me->IsFlying() && !_me->GetInstanceId() && unit->GetObjectTypeID() == ID_PLAYER && !(unit->ToPlayer()->GetDelayedOperations() & DELAYED_VEHICLE_TELEPORT) && _me->GetEntry() != 30275 /*NPC_WILD_WYRM*/)
        _me->CastSpell(unit, VEHICLE_SPELL_PARACHUTE, true);

    if (_me->GetObjectTypeID() == ID_UNIT)
        sScriptMgr->OnRemovePassenger(this, unit);

    if (_me->GetObjectTypeID() == ID_UNIT && _me->ToCreature()->IsAIEnabled)
...
bool Vehicle::IsVehicleInUse()
{
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
        if (Unit* passenger = ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger.Guid))
        {
            if (passenger->GetObjectTypeID() == ID_PLAYER)
                return true;
            else if (passenger->GetObjectTypeID() == ID_UNIT && passenger->GetVehicleKit() && passenger->GetVehicleKit()->IsVehicleInUse())
                return true;
        }
...
void Vehicle::TeleportVehicle(float x, float y, float z, float ang)
{
    ...
    for (SeatMap::const_iterator itr = Seats.begin(); itr != Seats.end(); ++itr)
        if (Unit* passenger = ObjectAccessor::GetUnit(*GetBase(), itr->second.Passenger.Guid))
        {
            ...
            else if (passenger->GetObjectTypeID() == ID_UNIT && passenger->GetVehicleKit())
