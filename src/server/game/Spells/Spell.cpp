    ...
    , m_spellValue(new SpellValue(m_spellInfo)), _spellEvent(nullptr)
{
    ...
    if (m_attackType == RANGED_ATTACK)
        // wand case
        if ((m_caster->getClassMask() & CLASSMASK_WAND_USERS) != 0 && m_caster->GetObjectTypeID() == ID_PLAYER)
...
void Spell::InitExplicitTargets(SpellCastTargets const& targets)
{
    ...
    else
    {
        ...
        if (neededTargets & (TARGET_FLAG_UNIT_MASK))
        {
            ...
            else if ((m_caster->GetObjectTypeID() == ID_UNIT) && neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT))
...
void Spell::SelectImplicitCasterObjectTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_UNIT_PASSENGER_7:
            if (m_caster->GetObjectTypeID() == ID_UNIT && m_caster->ToCreature()->IsVehicle())
...
void Spell::SelectEffectTypeImplicitTargets(uint8 effIndex)
{
    ...
    switch (m_spellInfo->Effects[effIndex].Effect)
    {
        ...
        case SPELL_EFFECT_SUMMON_PLAYER:
            if (m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->ToPlayer()->GetTarget())
            {
...
void Spell::prepareDataForTriggerSystem(AuraEffect const* /*triggeredByAura*/)
{
    ...
    if (m_originalCaster && m_caster != m_originalCaster && m_caster->GetObjectTypeID() == ID_UNIT && m_caster->ToCreature()->IsTotem() && m_caster->IsControlledByPlayer())
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    else if (missInfo == SPELL_MISS_REFLECT)                // In case spell reflect from target, do all effect on caster (if hit)
    {
        ...
        if (missInfo == SPELL_MISS_NONE)       // If reflected spell hit caster -> do all effect on him
        {
            ...
            unitTarget = m_caster;
            if (m_caster->GetObjectTypeID() == ID_UNIT)
    ...
    if (missInfo != SPELL_MISS_NONE && m_needComboPoints && m_targets.GetUnitTargetGUID() == target->targetGUID)
    {
        ...
        if (m_caster->GetObjectTypeID() == ID_PLAYER && (missInfo == SPELL_MISS_MISS || missInfo == SPELL_MISS_DODGE || missInfo == SPELL_MISS_PARRY))
    ...
    else if (m_damage > 0)
    {
        ...
        if (canEffectTrigger)
        {
            ...
            if (caster->GetObjectTypeID() == ID_PLAYER && m_spellInfo->HasAttribute(SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT) == 0 &&
    ...
    else
    {
        ...
        if (canEffectTrigger)
        {
            ...
            if ((procEx & PROC_EX_NORMAL_HIT || procEx & PROC_EX_CRITICAL_HIT) && caster->GetObjectTypeID() == ID_PLAYER && m_spellInfo->HasAttribute(SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT) == 0 &&
                    m_spellInfo->HasAttribute(SPELL_ATTR4_SUPRESS_WEAPON_PROCS) == 0 && (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MELEE || m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_RANGED))
                caster->ToPlayer()->CastItemCombatSpell(unitTarget, m_attackType, procVictim | PROC_FLAG_TAKEN_DAMAGE, procEx);
        }

        // Failed Pickpocket, reveal rogue
        if (missInfo == SPELL_MISS_RESIST && m_spellInfo->HasAttribute(SPELL_ATTR0_CU_PICKPOCKET) && unitTarget->GetObjectTypeID() == ID_UNIT && m_caster)
        {
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR7_CAN_CAUSE_INTERRUPT) && effectUnit->GetObjectTypeID() != ID_PLAYER)
        caster->CastSpell(effectUnit, SPELL_INTERRUPT_NONPLAYER, true);

    if (spellHitTarget)
    {
        //AI functions
        if (spellHitTarget->GetObjectTypeID() == ID_UNIT)
        {
            if (spellHitTarget->ToCreature()->IsAIEnabled)
                spellHitTarget->ToCreature()->AI()->SpellHit(m_caster, m_spellInfo);
        }

        if (m_caster->GetObjectTypeID() == ID_UNIT && m_caster->ToCreature()->IsAIEnabled)
        ...
        if (effectUnit->IsPvP() && effectUnit != m_caster && effectUnit->GetOwnerGUID() != m_caster->GetGUID() &&
            m_caster->GetObjectTypeID() == ID_PLAYER && !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_NO_PVP_FLAG))
        {
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster->GetObjectTypeID() == ID_PLAYER)
    {
    ...
    if (m_caster != unit)
    {
        ...
        if (m_spellInfo->Speed > 0.0f)
        {
            if (unit->GetObjectTypeID() == ID_UNIT && unit->ToCreature()->IsInEvadeMode())
                return SPELL_MISS_EVADE;
        ...
        else if (m_caster->IsFriendlyTo(unit))
        {
            ...
            if(!IsTriggered() && m_spellInfo->Speed > 0.0f && unit->GetObjectTypeID() == ID_PLAYER && !m_spellInfo->IsPositive())
                return SPELL_MISS_EVADE;
            ...
            if (unit->HasUnitState(UNIT_STATE_ATTACK_PLAYER))
            {
                ...
                if (m_caster->GetObjectTypeID() == ID_PLAYER && !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_NO_PVP_FLAG))
                    m_caster->ToPlayer()->UpdatePvP(true);
            }
    ...
    if (((m_spellFlags & SPELL_FLAG_REFLECTED) && !(unit->HasAuraType(SPELL_AURA_REFLECT_SPELLS))) || (aura_effmask && m_diminishGroup && unit != m_caster && (m_caster->GetObjectTypeID() != ID_UNIT || !m_caster->ToCreature()->isWorldBoss())))
    {
        m_diminishLevel = unit->GetDiminishing(m_diminishGroup);
        DiminishingReturnsType type = GetDiminishingReturnsGroupType(m_diminishGroup);

        uint32 flagsExtra = unit->GetObjectTypeID() == ID_UNIT ? unit->ToCreature()->GetCreatureTemplate()->flags_extra : 0;
...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (result != SPELL_CAST_OK && !IsAutoRepeat())          //always cast autorepeat dummy for triggering
    {
        ...
        if (m_caster->GetObjectTypeID() == ID_PLAYER && triggeredByAura && triggeredByAura->IsPeriodic() && !triggeredByAura->GetBase()->IsPassive())
        {
            SendChannelUpdate(0);
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) && m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->isMoving() && m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT && !IsTriggered())
    {
    ...
    else
    {
        ...
        {
            if (m_caster->GetObjectTypeID() == ID_UNIT && !m_caster->ToCreature()->IsInEvadeMode() &&
...
void Spell::cancel(bool bySelf)
{
    ...
    switch (oldState)
    {
        ...
        case SPELL_STATE_CASTING:
            ...
            if (m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->ToPlayer()->NeedSendSpectatorData())
                ArenaSpectator::SendCommand_Spell(m_caster->FindMap(), m_caster->GetGUID(), "SPE", m_spellInfo->Id, bySelf ? 99998 : 99999);
...
void Spell::_cast(bool skipCheck)
{
    ...
    {
        if (m_caster->GetObjectTypeID() == ID_PLAYER && !m_spellInfo->HasEffect(SPELL_EFFECT_SUMMON_PET))
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_SET_FACING))
        if (m_caster->GetObjectTypeID() == ID_UNIT && m_targets.GetObjectTarget() && m_caster != m_targets.GetObjectTarget())
    ...
    if (Unit* target = m_targets.GetUnitTarget())
        if (m_caster->GetObjectTypeID() == ID_PLAYER || (m_caster->IsPet() && m_caster->IsControlledByPlayer()))
        ...
        if (m_caster->GetCharmerOrOwnerPlayerOrPlayerItself())
            for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
            {
                ...
                if (ihit->targetGUID == m_caster->GetGUID() && ihit->missCondition != SPELL_MISS_NONE)
                    if (m_caster->GetObjectTypeID() == ID_PLAYER && m_spellInfo->IsCooldownStartedOnEvent())
...
void Spell::update(uint32 difftime)
{
    ...
    if ((m_caster->GetObjectTypeID() == ID_PLAYER && m_timer != 0) &&
...
void Spell::finish(bool ok)
{
    ...
    if (m_spellInfo->IsChanneled() && m_caster->GetObjectTypeID() == ID_PLAYER)
    {
        if (Unit* charm = m_caster->GetCharm())
    ...
    if (m_caster->GetObjectTypeID() == ID_UNIT && m_caster->ToCreature()->IsSummon())
    {
        // Unsummon statue
    ...
    if (m_caster->GetObjectTypeID() == ID_PLAYER && !m_triggeredByAuraSpell)
...
void Spell::SendCastResult(SpellCastResult result)
{
    ...
    if (m_caster->GetObjectTypeID() != ID_PLAYER || m_caster->IsCharmed())
        return;
...
void Spell::SendSpellStart()
{
    ...
    if (m_caster->GetObjectTypeID() == ID_PLAYER || m_caster->IsPet())
    {
    ...
    if (!m_spellInfo->IsChanneled() && m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->ToPlayer()->NeedSendSpectatorData())
...
void Spell::SendResurrectRequest(Player* target)
{
    ...
    std::string const sentName(m_caster->GetObjectTypeID() == ID_PLAYER
                               ? ""
    ...
    data << uint8(m_caster->GetObjectTypeID() == ID_PLAYER ? 0 : 1); // "you'll be afflicted with resurrection sickness"
...
{
    if (!m_CastItem || m_caster->GetObjectTypeID() != ID_PLAYER)
        return;
...
void Spell::TakeAmmo()
{
    if (m_attackType == RANGED_ATTACK && m_caster->GetObjectTypeID() == ID_PLAYER)
    {
        Item* pItem = m_caster->ToPlayer()->GetWeaponForAttack(RANGED_ATTACK);
...
{
    if (m_caster->GetObjectTypeID() != ID_PLAYER || m_caster->getClass() != CLASS_DEATH_KNIGHT)
        return;
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (!m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE))
    {
        ...
        else if (!IsTriggered() && m_caster->GetObjectTypeID() == ID_UNIT && m_caster->ToCreature()->IsSpellProhibited(m_spellInfo->GetSchoolMask()))
            return SPELL_FAILED_NOT_READY;
    }
    ...
    if (!IsTriggered() && m_caster->GetObjectTypeID() == ID_PLAYER)
        if (Battleground* bg = m_caster->ToPlayer()->GetBattleground())
            if (bg->GetStatus() == STATUS_WAIT_LEAVE)
                return SPELL_FAILED_DONT_REPORT;

    if (m_caster->GetObjectTypeID() == ID_PLAYER /*&& VMAP::VMapFactory::createOrGetVMapMgr()->isLineOfSightCalcEnabled()*/) // pussywizard: optimization (commented)
    {
    ...
    {
        if (m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->GetMap()->IsDungeon())
    ...
    if (m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->ToPlayer()->isMoving() && !IsTriggered())
    {
    ...
    if (vehicle && !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE))
    {
        ...
        if (!m_spellInfo->HasAttribute(SPELL_ATTR6_ALLOW_WHILE_RIDING_VEHICLE) && !m_spellInfo->HasAttribute(SPELL_ATTR0_ALLOW_WHILE_MOUNTED)
                && (vehicleSeat->m_flags & checkMask) != checkMask && m_caster->GetObjectTypeID() == ID_PLAYER)
            return SPELL_FAILED_DONT_REPORT;
    }
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR3_ONLY_BATTLEGROUNDS) &&  m_caster->GetObjectTypeID() == ID_PLAYER)
    ...
    // zone check
    if (m_caster->GetObjectTypeID() == ID_UNIT || !m_caster->ToPlayer()->IsGameMaster())
    {
        uint32 zone, area;
        m_caster->GetZoneAndAreaId(zone, area);

        SpellCastResult locRes = m_spellInfo->CheckLocation(m_caster->GetMapId(), zone, area,
                                 m_caster->GetObjectTypeID() == ID_PLAYER ? m_caster->ToPlayer() : nullptr);
        if (locRes != SPELL_CAST_OK)
            return locRes;
    }

    // not let players cast spells at mount (and let do it to creatures)
    if (m_caster->IsMounted() && m_caster->GetObjectTypeID() == ID_PLAYER && !(_triggeredCastFlags & TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE) &&
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_SKINNING:
                {
                    if (m_caster->GetObjectTypeID() != ID_PLAYER || !m_targets.GetUnitTarget() || m_targets.GetUnitTarget()->GetObjectTypeID() != ID_UNIT)
                        return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_EFFECT_OPEN_LOCK:
                {
                    ...
                    if (m_caster->GetObjectTypeID() != ID_PLAYER  // only players can open locks, gather etc.
            ...
            case SPELL_EFFECT_CREATE_TAMED_PET:
                {
                    if (m_targets.GetUnitTarget())
                    {
                        if (m_targets.GetUnitTarget()->GetObjectTypeID() != ID_PLAYER)
                            return SPELL_FAILED_BAD_TARGETS;
            ...
            case SPELL_EFFECT_SUMMON_PET:
                {
                    ...
                    if (m_caster->GetObjectTypeID() == ID_PLAYER && m_caster->getClass() == CLASS_WARLOCK && strict)
                        if (Pet* pet = m_caster->ToPlayer()->GetPet())
            ...
            case SPELL_AURA_AOE_CHARM:
                {
                    ...
                    if (Unit* target = m_targets.GetUnitTarget())
                    {
                        if (target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->IsVehicle())
            ...
            case SPELL_AURA_MOUNTED:
                {
                    ...
                    if (m_caster->GetObjectTypeID() == ID_PLAYER && !allowMount && !m_spellInfo->AreaGroupId)
            ...
            case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
                {
                    ...
                    if (m_originalCaster && m_originalCaster->GetObjectTypeID() == ID_PLAYER && m_originalCaster->IsAlive())
                    {
            ...
            case SPELL_AURA_PERIODIC_MANA_LEECH:
                {
                    ...
                    if (m_caster->GetObjectTypeID() != ID_PLAYER || m_CastItem)
                        break;
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (min_range && GetCaster()->GetObjectTypeID() == ID_UNIT && !GetCaster()->GetOwnerGUID().IsPlayer() && min_range <= 6.0f)
    ...
    if (target)
    {
        if (target != m_caster)
        {
            ...
            if (range_type == SPELL_RANGE_MELEE)
            {
                ...
                if (m_caster->GetObjectTypeID() != ID_UNIT && m_caster->isMoving() && target->isMoving() && !m_caster->IsWalking() && !target->IsWalking())
            ...
            if (m_caster->GetObjectTypeID() == ID_PLAYER && (m_spellInfo->FacingCasterFlags & SPELL_FACING_FLAG_INFRONT) && !m_caster->HasInArc(static_cast<float>(M_PI), target))
...
SpellCastResult Spell::CheckItems()
{
    ...
    else
    {
        ...
        if (m_caster->GetObjectTypeID() == ID_PLAYER && !m_caster->ToPlayer()->HasItemFitToSpellRequirements(m_spellInfo))
            return SPELL_FAILED_EQUIPPED_ITEM_CLASS;
    }
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM_2:
            {
                ...
                if (target->GetObjectTypeID() == ID_PLAYER && !IsTriggered())
                {
...
{
    if (!m_caster)// || m_caster->GetObjectTypeID() != ID_PLAYER)
        return;
...
{
    if (!m_caster || m_caster->GetObjectTypeID() != ID_PLAYER || getState() != SPELL_STATE_CASTING)
        return;
...
bool Spell::UpdatePointers()
{
    ...
    if (m_castItemGUID && m_caster->GetObjectTypeID() == ID_PLAYER)
    {
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
    switch (m_spellInfo->Effects[eff].ApplyAuraName)
    {
        ...
        case SPELL_AURA_MOD_POSSESS_PET:
        case SPELL_AURA_AOE_CHARM:
            if (target->GetObjectTypeID() == ID_UNIT && target->IsVehicle())
                return false;
            if (target->IsMounted())
                return false;
            if (target->GetCharmerGUID())
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) || (target->GetObjectTypeID() == ID_UNIT && target->HasUnitFlag(UNIT_FLAG_NOT_SELECTABLE) && (m_spellInfo->Effects[eff].TargetA.GetCheckType() == TARGET_CHECK_ENTRY || m_spellInfo->Effects[eff].TargetB.GetCheckType() == TARGET_CHECK_ENTRY)))
        return true;
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        ...
        case SPELL_EFFECT_SKIN_PLAYER_CORPSE:
            ...
            break;
        case SPELL_EFFECT_SUMMON_RAF_FRIEND:
            if (m_caster->GetObjectTypeID() != ID_PLAYER || target->GetObjectTypeID() != ID_PLAYER)
                return false;
...
SpellEvent::~SpellEvent()
{
    ...
    else
    {
        LOG_ERROR("spells", "~SpellEvent: {} {} tried to delete non-deletable spell {}. Was not deleted, causes memory leak.",
                       (m_Spell->GetCaster()->GetObjectTypeID() == ID_PLAYER ? "Player" : "Creature"), m_Spell->GetCaster()->GetGUID().ToString(), m_Spell->m_spellInfo->Id);
        ABORT();
    }
}
            ...
            case LOCK_KEY_SKILL:
                {
                    ...
                    if (skillId != SKILL_NONE)
                    {
                        ...
                        skillValue = m_CastItem || m_caster->GetObjectTypeID() != ID_PLAYER ?
...
bool Spell::HasGlobalCooldown() const
{
    ...
    else if (m_caster->GetObjectTypeID() == ID_PLAYER)
...
namespace Acore
{
    ...
    {
        if (target->GetObjectTypeID() == ID_GAMEOBJECT)
        {
            ...
            return false;
        else if (target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->IsAvoidingAOE()) // pussywizard
            return false;
