...
struct npc_love_in_air_supply_sentry : public ScriptedAI
{
    ...
    {
        if (lock > 1000 && me->GetDistance(who) < 10.0f && who->GetTypeId() == ID_PLAYER && who->HasAura(SPELL_GOBLIN_DISGUISE) && !who->HasAura(SPELL_GOBLIN_CARRY_CRATE))
        {
            lock = 0;
            if (urand(0, 1))
            {
...
struct npc_love_in_air_snivel : public NullCreatureAI
{
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (delay == 0 && me->GetDistance(who) < 7.0f && who->GetTypeId() == ID_PLAYER)
        {
