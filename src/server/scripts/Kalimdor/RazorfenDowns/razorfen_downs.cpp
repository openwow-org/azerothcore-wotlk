...
public:
    ...
    struct npc_idol_room_spawnerAI : public NullCreatureAI
    {
        ...
        void SetData(uint32 /*type*/, uint32 data) override
        {
            if (data < 7)
            {
                me->SummonCreature(NPC_WITHERED_BATTLE_BOAR, me->GetPositionX(),  me->GetPositionY(),  me->GetPositionZ(),  me->GetFacing());
                if (data > 0 && me->GetFacing() < 4.0f)
                    me->SummonCreature(NPC_WITHERED_BATTLE_BOAR, me->GetPositionX(),  me->GetPositionY(),  me->GetPositionZ(),  me->GetFacing());
                me->SummonCreature(NPC_DEATHS_HEAD_GEOMANCER, me->GetPositionX() + (cos(me->GetFacing() - (M_PI / 2)) * 2), me->GetPositionY() + (std::sin(me->GetFacing() - (M_PI / 2)) * 2), me->GetPositionZ(),  me->GetFacing());
                me->SummonCreature(NPC_WITHERED_QUILGUARD, me->GetPositionX() + (cos(me->GetFacing() + (M_PI / 2)) * 2), me->GetPositionY() + (std::sin(me->GetFacing() + (M_PI / 2)) * 2), me->GetPositionZ(),  me->GetFacing());
            }
            else if (data == 7)
                me->SummonCreature(NPC_PLAGUEMAW_THE_ROTTING, me->GetPositionX(),  me->GetPositionY(),  me->GetPositionZ(),  me->GetFacing());
        }
    };
