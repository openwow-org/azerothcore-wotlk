        ...
        case TARGET_DEST_CASTER_FRONT_LEAP:
            {
                ...
                if (!m_caster->HasUnitMovementFlag(MOVEFLAG_FALLING) || (pos.GetPositionZ() - ground < distance))
                {
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit && m_caster->IsEnemy(unit) && !m_spellInfo->IsPositive() && !m_triggeredByAuraSpell && !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_DONT_BREAK_STEALTH))
    {
...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (!(m_spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED) && !(m_spellInfo->Attributes & SPELL_ATTR0_ALLOW_WHILE_SITTING) && !m_triggeredByAuraSpell && m_caster->IsSitting())
    {
