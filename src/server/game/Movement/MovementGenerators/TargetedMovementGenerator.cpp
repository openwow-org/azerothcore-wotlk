...
bool ChaseMovementGenerator<T>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    bool forceDest =
        ...
        (i_target->GetObjectTypeID() == ID_PLAYER && i_target->ToPlayer()->IsGameMaster()) || // for .npc follow
        (owner->CanFly())
...
bool FollowMovementGenerator<T>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    if (PositionOkay(target, owner->IsGuardian() && target->GetObjectTypeID() == ID_PLAYER, targetIsMoving, time_diff))
    {
