...
bool ThreatCalcHelper::isValidProcess(Unit* hatedUnit, Unit* hatingUnit, SpellInfo const* threatSpell)
{
    ...
    // not to GM
    if (hatedUnit->GetTypeId() == ID_PLAYER && hatedUnit->ToPlayer()->IsGameMaster())
        return false;
    ...
    ASSERT(hatingUnit->GetTypeId() == ID_UNIT);
...
void HostileReference::updateOnlineStatus()
{
    ...
    if (isValid()
            && (getTarget()->GetTypeId() != ID_PLAYER || !getTarget()->ToPlayer()->IsGameMaster())
...
void ThreatMgr::_addThreat(Unit* victim, float threat)
{
    ...
    if (!ref) // there was no ref => create a new one
    {
        ...
        if (victim->GetTypeId() == ID_PLAYER && victim->ToPlayer()->IsGameMaster())
