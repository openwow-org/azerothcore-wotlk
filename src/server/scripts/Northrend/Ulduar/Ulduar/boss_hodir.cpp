...
public:
    ...
    struct boss_hodirAI : public ScriptedAI
    {
        ...
        {
            if (t->GetObjectTypeID() == ID_PLAYER)
                return !t->HasAura(SPELL_FLASH_FREEZE_TRAPPED_PLAYER);
            else if (t->GetObjectTypeID() == ID_UNIT)
        ...
        void UpdateAI(uint32 diff) override
        {
            if (timer <= diff)
            {
                ...
                if (me->IsSummon())
                {
                    ...
                    {
                        if ((s->GetObjectTypeID() == ID_PLAYER && !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_PLAYER)) || (s->GetObjectTypeID() == ID_UNIT && !s->HasAura(SPELL_FLASH_FREEZE_TRAPPED_NPC)))
                            me->DespawnOrUnsummon(2000);
                        else if (s->GetObjectTypeID() == ID_PLAYER)
    ...
    class spell_hodir_flash_freeze_AuraScript : public AuraScript
    {
        ...
        void HandleEffectPeriodic(AuraEffect const* aurEff)
        {
            if (aurEff->GetTotalTicks() > 0 && aurEff->GetTickNumber() == uint32(aurEff->GetTotalTicks()) - 1)
            {
                ...
                if (!target || !caster || caster->GetObjectTypeID() != ID_UNIT)
                    return;

                if (Aura* aur = target->GetAura(target->GetObjectTypeID() == ID_PLAYER ? SPELL_FLASH_FREEZE_TRAPPED_PLAYER : SPELL_FLASH_FREEZE_TRAPPED_NPC))
                {
                    if (Unit* caster2 = aur->GetCaster())
                    {
                        if (caster2->GetObjectTypeID() == ID_UNIT)
                        {
                ...
                else if (target->GetObjectTypeID() == ID_UNIT)
                {
...
public:
    ...
    {
        return target && target->GetEntry() == NPC_HODIR && target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->AI()->GetData(1);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->AI()->GetData(2);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->AI()->GetData(3);
    }
};
...
public:
    ...
    bool OnCheck(Player*  /*player*/, Unit* target, uint32 /*criteria_id*/) override
    {
        return target && target->GetEntry() == NPC_HODIR && target->GetObjectTypeID() == ID_UNIT && target->ToCreature()->AI()->GetData(4);
    }
};
