...
public:
    ...
    struct npc_ulduar_expedition_commanderAI : public NullCreatureAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            if (who->GetObjectTypeID() != ID_PLAYER || me->GetExactDist2d(who) > 15.0f)
                return;
...
public:
    ...
    {
        return target && target->GetObjectTypeID() == ID_UNIT && target->GetEntry() == NPC_RAZORSCALE && target->ToCreature()->AI()->GetData(1);
    }
};
