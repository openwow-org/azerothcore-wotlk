        ...
        for (GuidUnorderedSet::const_iterator itr = m_players[team].begin(); itr != m_players[team].end(); ++itr)
        {
            ...
            if (Player* player = ObjectAccessor::FindPlayer(*itr))
            {
                ...
                if (5500 > x && x > 5392 && y < 2880 && y > 2800 && z < 480)
                    player->Teleport(571, 5349.8686f, 2838.481f, 409.240f, 0.046328f);
...
void BattlefieldWG::OnPlayerJoinWar(Player* player)
{
    ...
    if (player->GetTeamId() == GetDefenderTeam())
        player->Teleport(571, 5345, 2842, 410, 3.14f);
    else
    {
        if (player->GetTeamId() == TEAM_HORDE)
            player->Teleport(571, 5025.857422f, 3674.628906f, 362.737122f, 4.135169f);
        else
            player->Teleport(571, 5101.284f, 2186.564f, 365.549f, 3.812f);
    }
