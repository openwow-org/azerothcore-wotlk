...
{
    if (!m_spellmod || target->GetTypeId() != ID_PLAYER)
        return;
...
void AuraEffect::HandleModInvisibilityDetect(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    target->UpdateObjectVisibility(target->GetTypeId() == ID_PLAYER || target->GetOwnerGUID().IsPlayer());
}
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        // apply glow vision
        if (target->GetTypeId() == ID_PLAYER && (type == INVISIBILITY_GENERAL || type == INVISIBILITY_UNK10))
    ...
    else
    {
        ...
        else
        {
            ...
            if (!found)
            {
                ...
                // remove glow vision
                if (target->GetTypeId() == ID_PLAYER && !target->m_invisibility.HasFlag(INVISIBILITY_GENERAL) && !target->m_invisibility.HasFlag(INVISIBILITY_UNK10))
                {
    ...
    target->UpdateObjectVisibility(target->GetTypeId() == ID_PLAYER || target->GetOwnerGUID().IsPlayer() || target->GetMap()->Instanceable(), true);
...
void AuraEffect::HandleAuraTransform(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        // polymorph case
        if ((mode & AURA_EFFECT_HANDLE_REAL) && target->GetTypeId() == ID_PLAYER && target->IsPolymorphed())
        {
        ...
        for (UnitList::iterator iter = targets.begin(); iter != targets.end(); ++iter)
        {
            ...
            for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
            {
                if ((*iter)->GetCurrentSpell(i) && (*iter)->GetCurrentSpell(i)->m_targets.GetUnitTargetGUID() == target->GetGUID())
                {
                    ...
                    if (si->HasAttribute(SPELL_ATTR6_IGNORE_PHASE_SHIFT) && (*iter)->GetTypeId() == ID_UNIT)
                    {
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == ID_PLAYER && (!target->IsInFeralForm() || target->GetShapeshiftForm() == FORM_GHOSTWOLF))
    {
    ...
    if (target->GetTypeId() == ID_UNIT && target->ToCreature()->GetCurrentEquipmentId())
...
void AuraEffect::HandleAuraModStalked(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // call functions which may have additional effects after chainging state of unit
    target->UpdateObjectVisibility(target->GetTypeId() == ID_PLAYER);
}
...
void AuraEffect::HandleAuraAllowFlight(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!apply && target->GetTypeId() == ID_UNIT && !target->IsLevitating())
...
void AuraEffect::HandleAuraFeatherFall(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!apply && target->GetTypeId() == ID_PLAYER)
...
void AuraEffect::HandlePreventFleeing(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == ID_PLAYER || !apply || target->HasAuraType(SPELL_AURA_MOD_FEAR))
        return;
...
void AuraEffect::HandleAuraModResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
    {
        if (GetMiscValue() & int32(1 << x))
        {
            ...
            if (target->GetTypeId() == ID_PLAYER || target->IsPet())
...
void AuraEffect::HandleModTargetResistance(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == ID_PLAYER && (GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
        target->ApplyModInt32Value(PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE, GetAmount(), apply);

    // show as spell penetration only full spell penetration bonuses (all resistances except armor and holy
    if (target->GetTypeId() == ID_PLAYER && (GetMiscValue() & SPELL_SCHOOL_MASK_SPELL) == SPELL_SCHOOL_MASK_SPELL)
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetId() == 67480) // xinef: hack fix for blessing of sanctuary stats stack with blessing of kings...
    {
        ...
        for (int32 i = STAT_STRENGTH; i < MAX_STATS; i++)
        {
            ...
            {
                if (apply && (target->GetTypeId() == ID_PLAYER || target->IsPet()))
...
void AuraEffect::HandleAuraModRangedAttackPowerOfStatPercent(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    // Recalculate bonus
    if (target->GetTypeId() == ID_PLAYER && !(target->getClassMask() & CLASSMASK_WAND_USERS))
...
void AuraEffect::HandleModDamageDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if ((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) != 0 && sScriptMgr->CanModAuraEffectDamageDone(this, target, aurApp, mode, apply))
    {
        ...
        if (GetSpellInfo()->EquippedItemClass == -1 || target->GetTypeId() != ID_PLAYER)
        {
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if ((GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL) && (GetSpellInfo()->EquippedItemClass == -1 || target->GetTypeId() != ID_PLAYER))
    {
            ...
            switch (m_spellInfo->SpellFamilyName)
            {
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        ...
                        case 43681: // Inactive
                            {
                                if (target->GetTypeId() != ID_PLAYER || aurApp->GetRemoveMode() != AURA_REMOVE_BY_EXPIRE)
                                    return;
                        ...
                        case 46374: // quest The Power of the Elements (11893)
                            {
                                if (target->isDead() && GetBase() && target->GetTypeId() == ID_UNIT && target->GetEntry() == 24601)
                                {
                                    auto caster2 = GetBase()->GetCaster();
                                    if (caster2 && caster2->GetTypeId() == ID_PLAYER)
                                    {
...
void AuraEffect::HandleAuraOpenStable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() != ID_PLAYER || !target->IsInWorld())
        return;
...
void AuraEffect::HandlePreventResurrection(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (aurApp->GetTarget()->GetTypeId() != ID_PLAYER)
        return;
            ...
            case SPELLFAMILY_GENERIC:
                {
                    switch (auraId)
                    {
                        ...
                        case 29528:
                            if (target->GetTypeId() != ID_UNIT) // prevent error reports in case ignored player target
                                return;
                            break;
                        ...
                        case 38554:
                            {
                                if (!caster || target->GetTypeId() != ID_UNIT)
                                    return;
...
void AuraEffect::HandlePeriodicEnergizeAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (target->GetTypeId() == ID_PLAYER && target->getPowerType() != PowerType && !m_spellInfo->HasAttribute(SPELL_ATTR7_ONLY_IN_SPELLBOOK_UNTIL_LEARNED))
        return;
