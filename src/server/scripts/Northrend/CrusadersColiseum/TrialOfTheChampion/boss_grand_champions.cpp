                            ...
                            for( Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr )
                                if( Player* plr = itr->GetSource() )
                                    if( me->GetExactDist(plr) <= 5.0f )
                                        if( Vehicle* v = plr->GetVehicle() )
                                            if( Unit* c = v->GetBase() )
                                                if( c->GetObjectTypeID() == ID_UNIT && c->ToCreature()->GetEntry() == (pInstance->GetData(DATA_TEAMID_IN_INSTANCE) == TEAM_HORDE ? VEHICLE_ARGENT_BATTLEWORG : VEHICLE_ARGENT_WARHORSE) )
                                                {
