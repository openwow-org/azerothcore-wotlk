...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& message)
{
    if (!IsGMAccount()) {
        SendNotification(LANG_PERMISSION_DENIED);
        return;
    ...
    auto eventTime      = message.read<uint32>();
    auto continentID    = message.read<uint32>();
    auto player         = message.read<ObjectGuid>();
    auto position       = message.read<G3D::Vector3>();
    auto facing         = message.read<float>();
