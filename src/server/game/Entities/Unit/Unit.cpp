...
void Unit::Update(uint32 p_time)
{
    ...
    // pussywizard:
    if (GetTypeId() != ID_PLAYER || (!ToPlayer()->IsBeingTeleported() && !bRequestForcedVisibilityUpdate))
    {
    ...
    if (IsInCombat() && (GetTypeId() == ID_PLAYER || ((IsPet() || HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN)) && IsControlledByPlayer())))
    {
...
public:
    ...
    bool operator()(Movement::MoveSpline::UpdateResult result)
    {
        if ((result & (Movement::MoveSpline::Result_NextSegment | Movement::MoveSpline::Result_JustArrived)) &&
                _unit->GetTypeId() == ID_UNIT && _unit->GetMotionMaster()->GetCurrentMovementGeneratorType() == ESCORT_MOTION_TYPE &&
...
void Unit::UpdateSplineMovement(uint32 t_diff)
{
    ...
    if (arrived)
    {
        ...
        if (movespline->HasAnimation() && GetTypeId() == ID_UNIT && IsAlive())
...
{
    if (!victim || !victim->IsAlive() || victim->IsInFlight() || (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsEvadingAttacks()))
    {
        if (absorb)
            *absorb += damage;
        damage = 0;
...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool /*allowGM*/, Spell const* damageSpell /*= nullptr*/)
{
    ...
    if (victim->GetTypeId() == ID_PLAYER && attacker != victim)
    {
    ...
    if (victim->GetTypeId() == ID_UNIT && attacker != victim && victim->IsPet() && victim->IsAlive())
    ...
    if (damagetype != NODAMAGE)
    {
        ...
        {
            if (victim != attacker && victim->GetTypeId() == ID_PLAYER)
            {
        ...
        for (AuraEffectList::iterator i = vCopyDamageCopy.begin(); i != vCopyDamageCopy.end(); ++i)
        {
            ...
            if (attacker && shareDamageTarget->GetTypeId() == ID_PLAYER)
            {
    ...
    if (victim->GetTypeId() == ID_PLAYER && victim->ToPlayer()->duel && damage >= (health - 1))
    {
    ...
    else if (victim->IsVehicle() && damage >= (health - 1) && victim->GetCharmer() && victim->GetCharmer()->GetTypeId() == ID_PLAYER)
    {
    ...
    if (health <= damage)
    {
        ...
        //if (attacker && victim->GetTypeId() == ID_PLAYER && victim != attacker)
    ...
    else
    {
        ...
        if (attacker && attacker->GetTypeId() == ID_PLAYER)
        {
        ...
        {
            if (victim != attacker && victim->GetTypeId() == ID_PLAYER) // does not support creature push_back
            {
...
void Unit::DealSpellDamage(SpellNonMeleeDamage* damageInfo, bool durabilityLoss, Spell const* spell /*= nullptr*/)
{
    ...
    if (!victim->IsAlive() || victim->IsInFlight() || (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsEvadingAttacks()))
        return;
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType, const bool sittingVictim)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
    {
        ...
        uint32 damage = 0;
        uint8 itemDamagesMask = (GetTypeId() == ID_PLAYER) ? (1 << i) : 0;
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    {
        return victim->IsAlive() && !victim->HasUnitState(UNIT_STATE_IN_FLIGHT) && (victim->GetTypeId() != ID_UNIT || !victim->ToCreature()->IsEvadingAttacks());
    };
    ...
    if ((damageInfo->damages[0].damage + damageInfo->damages[1].damage) && ((damageInfo->hitOutCome == MELEE_HIT_CRIT || damageInfo->hitOutCome == MELEE_HIT_CRUSHING || damageInfo->hitOutCome == MELEE_HIT_NORMAL || damageInfo->hitOutCome == MELEE_HIT_GLANCING) &&
                               GetTypeId() != ID_PLAYER && !ToCreature()->IsControlledByPlayer() && !victim->HasInArc(M_PI, this)
                               && (victim->GetTypeId() == ID_PLAYER || !victim->ToCreature()->isWorldBoss()) && !victim->IsVehicle()))
    {
...
uint32 Unit::CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, const uint32 damage, SpellInfo const* spellInfo, uint8 attackerLevel, WeaponAttackType /*attackType*/)
{
    ...
    if (attacker)
    {
        ...
        if (attacker->GetTypeId() == ID_PLAYER)
        {
            float bonusPct = 0;
...
void Unit::CalcAbsorbResist(DamageInfo& dmgInfo, bool Splited)
{
    ...
    if (!(schoolMask & SPELL_SCHOOL_MASK_NORMAL) && (!(schoolMask & SPELL_SCHOOL_MASK_HOLY) || victim->GetTypeId() == ID_UNIT) && (!spellInfo || (!spellInfo->HasAttribute(SPELL_ATTR0_CU_BINARY_SPELL) && !spellInfo->HasAttribute(SPELL_ATTR4_NO_CAST_LOG))))
    {
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType /*= BASE_ATTACK*/, bool extra /*= false*/, bool ignoreCasting /*= false*/)
{
    ...
    const bool sittingVictim = victim->GetTypeId() == ID_PLAYER && (victim->IsSitState() || victim->getStandState() == UNIT_STAND_STATE_SLEEP);
    ...
    if (meleeAttack)
    {
        ...
        if (GetTypeId() == ID_PLAYER && !m_Controlled.empty())
...
{
    if (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsEvadingAttacks())
    {
    ...
    if (victim->GetTypeId() == ID_PLAYER && !victim->HasInArc(M_PI, this) && !victim->HasAuraType(SPELL_AURA_IGNORE_HIT_DIRECTION))
    {
    ...
    else if (victim->GetTypeId() == ID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_DODGE))
    {
    ...
    else
    {
        ...
        if (victim->GetTypeId() == ID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY))
        {
            tmp = parry_chance;
        ...
        if (victim->GetTypeId() == ID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_BLOCK))
        {
            tmp = block_chance;
    ...
    if (getLevelForTarget(victim) >= victim->getLevelForTarget(this) + 4 &&
            ...
            !(GetTypeId() == ID_UNIT && ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRUSHING_BLOWS))
    {
    ...
    if (tmp > 0 && roll < (sum += tmp))
    {
        ...
        if (GetTypeId() == ID_UNIT && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRIT))
        {
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        LOG_DEBUG("entities.unit", "{} {} stopped attacking {} {}", (GetTypeId() == ID_PLAYER ? "Player" : "Creature"), GetGUID().ToString(), (victim->GetTypeId() == ID_PLAYER ? "player" : "creature"), victim->GetGUID().ToString());
    else
        LOG_DEBUG("entities.unit", "{} {} stopped attacking", (GetTypeId() == ID_PLAYER ? "Player" : "Creature"), GetGUID().ToString());
}
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (!victim->IsAlive() && victim->GetTypeId() != ID_PLAYER)
        return SPELL_MISS_NONE;
    ...
    if (GetTypeId() == ID_UNIT && ToCreature()->IsTrigger())
        thisLevel = std::max<int32>(thisLevel, spellInfo->SpellLevel);
    int32 levelDiff = int32(victim->getLevelForTarget(this)) - thisLevel;

    int32 MISS_CHANCE_MULTIPLIER;
    if (sWorld->getBoolConfig(CONFIG_MISS_CHANCE_MULTIPLIER_ONLY_FOR_PLAYERS) && GetTypeId() != ID_PLAYER) // keep it as it was originally (7 and 11)
    {
        MISS_CHANCE_MULTIPLIER = victim->GetTypeId() == ID_PLAYER ? 7 : 11;
    }
    else
    {
        MISS_CHANCE_MULTIPLIER = sWorld->getRate(
            victim->GetTypeId() == ID_PLAYER
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spell, bool CanReflect)
{
    ...
    if (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsEvadingAttacks() && !spell->HasAura(SPELL_AURA_CONTROL_VEHICLE) && !spell->HasAttribute(SPELL_ATTR0_CU_IGNORE_EVADE))
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, Spell const* spell, bool CanReflect)
{
    ...
    // Return evade for units in evade mode
    if (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsEvadingAttacks() && !spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE) &&
...
uint32 Unit::GetDefenseSkillValue(Unit const* target) const
{
    if (GetTypeId() == ID_PLAYER)
    {
        ...
        uint32 value = (target && target->GetTypeId() == ID_PLAYER)
...
void Unit::_UpdateAutoRepeatSpell()
{
    ...
    if ((GetTypeId() == ID_PLAYER && ToPlayer()->isMoving() && spellProto->Id != HUNTER_AUTOSHOOT) || IsNonMeleeSpellCast(false, false, true, spellProto->Id == HUNTER_AUTOSHOOT))
    {
        // cancel wand shoot
...
AuraApplication* Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    ...
    if (!IsAlive() && !aurSpellInfo->IsDeathPersistent() && !aurSpellInfo->IsAllowingDeadTarget() && (GetTypeId() != ID_PLAYER || !ToPlayer()->GetSession()->PlayerLoading()))
        return nullptr;
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if (GetTypeId() == ID_PLAYER && gameObj->GetSpellId())
    {
...
void Unit::SendSpellNonMeleeReflectLog(SpellNonMeleeDamage* log, Unit* attacker)
{
    ...
    if (log->target->GetTypeId() == ID_PLAYER && log->target->ToPlayer()->GetCommandStatus(CHEAT_GOD))
    {
        absorb = damage;
        damage = 0;
    }
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
            {
                ...
                if (GetTypeId() == ID_PLAYER && ToPlayer()->GetCommandStatus(CHEAT_GOD))
                {
                    absorb = damage;
                    damage = 0;
                }
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
    {
        ...
        if (damageInfo->target->GetTypeId() == ID_PLAYER && damageInfo->target->ToPlayer()->GetCommandStatus(CHEAT_GOD))
        {
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, ProcEventInfo const& eventInfo)
{
    ...
    Item* castItem = triggeredByAura->GetBase()->GetCastItemGUID() && GetTypeId() == ID_PLAYER
        ...
        case SPELLFAMILY_GENERIC:
            {
                switch (dummySpell->Id)
                {
                    ...
                    case 39446:
                        {
                            if (GetTypeId() != ID_PLAYER || !IsAlive())
                                return false;
                    ...
                    case 71519: // Deathbringer's Will Normal
                        {
                            if (GetTypeId() != ID_PLAYER || HasSpellCooldown(71484))
                                return false;
                    ...
                    case 71562: // Deathbringer's Will Heroic
                        {
                            if (GetTypeId() != ID_PLAYER || HasSpellCooldown(71561))
                                return false;
        ...
        case SPELLFAMILY_DRUID:
            {
                    ...
                    case 24932:
                        {
                            ...
                            if (GetTypeId() == ID_PLAYER && !ToPlayer()->HasSpellCooldown(34299))
                ...
                // Eclipse
                if (dummySpell->SpellIconID == 2856 && GetTypeId() == ID_PLAYER)
                {
        ...
        case SPELLFAMILY_SHAMAN:
            {
                ...
                {
                    if (GetTypeId() != ID_PLAYER  || !victim || !victim->IsAlive() || !castItem || !castItem->IsEquipped())
                        return false;
                ...
                {
                    if(!procSpell || GetTypeId() != ID_PLAYER || !victim)
                        return false;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                // Sudden Doom
                if (dummySpell->SpellIconID == 1939 && GetTypeId() == ID_PLAYER)
                {
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown, uint32 procPhase, ProcEventInfo& eventInfo)
{
            ...
            case SPELLFAMILY_HUNTER:
                {
                    ...
                    {
                        if (GetTypeId() != ID_PLAYER || !ToPlayer()->GetPet())
                            return false;
            ...
            case SPELLFAMILY_PALADIN:
                {
                    switch (auraSpellInfo->Id)
                    {
                        ...
                        case 67758: // Item - Coliseum 25 Heroic Caster Trinket
                            {
                                if (!victim || !victim->IsAlive() || GetTypeId() != ID_PLAYER)
                                    return false;
                                ...
                                if (cooldown && GetTypeId() == ID_PLAYER)
                                {
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 62821:
            if (this->GetTypeId() != ID_PLAYER) // spell has Attribute, but persistent area auras ignore it
                return false;
            break;
    ...
    if (auraSpellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT)
    {
        ...
        {
            if (GetTypeId() != ID_PLAYER || getClass() != CLASS_DEATH_KNIGHT)
                return false;
        ...
        case 15250: // Rogue Setup
            {
                ...
                if (!victim || (GetTypeId() == ID_PLAYER && victim != ToPlayer()->GetSelectedUnit()))
                    return false;
...
bool Unit::Attack(Unit* victim, bool meleeAttack)
{
    ...
    if (GetTypeId() == ID_PLAYER && IsMounted())
        return false;
...
void Unit::CombatStop(bool includingCast)
{
    ...
    // xinef: just in case
    if (IsPetInCombat() && GetTypeId() != ID_PLAYER)
        ClearInPetCombat();
}
            ...
            if (RemoveGuidValue(UNIT_FIELD_SUMMON, minion->GetGUID()))
            {
                ...
                for (ControlSet::iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); ++itr)
                {
                    ...
                    if (AddGuidValue(UNIT_FIELD_SUMMON, (*itr)->GetGUID()))
                    {
                        ...
                        if (GetTypeId() == ID_PLAYER && !GetCharmGUID())
                        {
                            if ((*itr)->IsPet())
...
void Unit::GetAllMinionsByEntry(std::list<Creature*>& Minions, uint32 entry)
{
    for (Unit::ControlSet::iterator itr = m_Controlled.begin(); itr != m_Controlled.end();)
    {
        Unit* unit = *itr;
        ++itr;
        if (unit->GetEntry() == entry && unit->GetTypeId() == ID_UNIT
...
int32 Unit::DealHeal(Unit* healer, Unit* victim, uint32 addhealth)
{
    ...
    if (healer && healer->GetTypeId() == ID_UNIT && healer->ToCreature()->IsTotem())
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    else if (GetTypeId() == ID_UNIT && IsPet())
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint8 effIndex, float TotalMod, uint32 stack)
{
    ...
    if (GetTypeId() == ID_UNIT && (!ToCreature()->IsPet() || !ToCreature()->IsGuardian() || !ToCreature()->IsControlledByPlayer()))
...
float Unit::SpellDoneCritChance(Unit const* /*victim*/, SpellInfo const* spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType, bool skipEffectCheck) const
{
    ...
    if (GetTypeId() == ID_UNIT && !GetSpellModOwner())
        return -100.0f;
...
float Unit::SpellTakenCritChance(Unit const* caster, SpellInfo const* spellProto, SpellSchoolMask schoolMask, float doneChance, WeaponAttackType attackType, bool skipEffectCheck) const
{
    ...
    switch (spellProto->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
            ...
            if (GetTypeId() == ID_PLAYER && (IsSitState() || getStandState() == UNIT_STAND_STATE_SLEEP))
            {
                return 100.0f;
            }
            [[fallthrough]]; /// @todo: Not sure whether the fallthrough was a mistake (forgetting a break) or intended. This should be double-checked.
...
void Unit::Dismount()
{
    ...
    if (GetTypeId() == ID_PLAYER && GetVehicleKit())
    {
    ...
    if (Player* player = ToPlayer())
    {
        ...
        if (Unit* charm = player->GetCharm())
            if (charm->GetTypeId() == ID_UNIT && !charm->HasUnitState(UNIT_STATE_STUNNED))
...
void Unit::SetInCombatWith(Unit* enemy, uint32 duration)
{
    ...
    if (enemy->GetTypeId() == ID_UNIT && enemy->ToCreature()->IsTrigger())
        return;
    ...
    // check for duel
    if (eOwner->GetTypeId() == ID_PLAYER && eOwner->ToPlayer()->duel)
    {
...
{
    // Xinef: Dont allow to start combat with triggers
    if (victim->GetTypeId() == ID_UNIT && victim->ToCreature()->IsTrigger())
        return;
    ...
    if (initialAggro)
    {
        ...
        if (!victim->IsInCombat() && victim->GetTypeId() != ID_PLAYER && !victim->ToCreature()->HasReactState(REACT_PASSIVE) && victim->ToCreature()->IsAIEnabled)
        {
            if (victim->IsPet())
    ...
    Player* player = GetCharmerOrOwnerPlayerOrPlayerItself();
    if (player && who->IsPvP() && (who->GetTypeId() != ID_PLAYER || !player->duel || player->duel->Opponent != who))
    {
...
{
    // Xinef: Dont allow to start combat with triggers
    if (target->GetTypeId() == ID_UNIT && target->ToCreature()->IsTrigger())
        return;
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
    ...
    if (GetTypeId() == ID_UNIT && enemy && IsImmuneToPC() && enemy->GetCharmerOrOwnerPlayerOrPlayerItself())
    ...
    for (Unit::ControlSet::iterator itr = m_Controlled.begin(); itr != m_Controlled.end();)
    {
        ...
        if (controlled->GetTypeId() == ID_UNIT && controlled->ToCreature()->HasReactState(REACT_PASSIVE))
            continue;
...
bool Unit::isTargetableForAttack(bool checkFakeDeath, Unit const* byWho) const
{
    ...
    if (GetTypeId() == ID_PLAYER && ToPlayer()->IsGameMaster())
        return false;
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (target->HasUnitState(UNIT_STATE_UNATTACKABLE)
            || (target->GetTypeId() == ID_PLAYER && target->ToPlayer()->IsGameMaster()))
        return false;
    ...
    if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_IGNORE_PHASE_SHIFT)) && (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea()) : !CanSeeOrDetect(target, (bySpell && bySpell->IsAffectingArea()) || (target->GetTypeId() == ID_PLAYER && target->HasStealthAura() && target->IsInCombat() && IsInCombatWith(target)))))
        return false;
            ...
            repTargetToThis <= REP_NEUTRAL)
    {
        ...
        if  (!(target->GetTypeId() == ID_PLAYER && thisUnit->GetTypeId() == ID_PLAYER) &&
                !(target->GetTypeId() == ID_UNIT && thisUnit->GetTypeId() == ID_UNIT))
        {
            Player const* player = target->GetTypeId() == ID_PLAYER ? target->ToPlayer() : thisUnit->ToPlayer();
            Unit const* creature = target->GetTypeId() == ID_UNIT ? target : thisUnit;
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
            {
                if (GetTypeId() == ID_UNIT && IsControlledByPlayer()) // not sure if good for pet
                {
                ...
                if (GetTypeId() == ID_PLAYER && GetVehicle())
    ...
    {
        float base_speed = (GetTypeId() == ID_UNIT ? ToCreature()->GetCreatureTemplate()->speed_run : 1.0f);
    ...
    else
    {
        if (GetTypeId() == ID_PLAYER)
        {
            ...
            if (!IsInCombat())
            {
                ...
                if (pet && pet->GetTypeId() == ID_UNIT && !pet->IsInCombat() && pet->GetMotionMaster()->GetCurrentMovementGeneratorType() == FOLLOW_MOTION_TYPE)
...
{
    ASSERT(GetTypeId() == ID_UNIT);

    if (!taunter || (taunter->GetTypeId() == ID_PLAYER && taunter->ToPlayer()->IsGameMaster()))
        return;
...
Unit* Creature::SelectVictim()
{
    ...
    for (AttackerSet::const_iterator itr = m_attackers.begin(); itr != m_attackers.end(); ++itr)
        if ((*itr) && CanCreatureAttack(*itr) && (*itr)->GetTypeId() != ID_PLAYER && !(*itr)->ToCreature()->HasUnitTypeMask(UNIT_MASK_CONTROLABLE_GUARDIAN))
            return nullptr;
...
float Unit::ApplyDiminishingToDuration(DiminishingGroup group, int32& duration, Unit* caster, DiminishingLevels Level, int32 limitduration)
{
    ...
    {
        if (GetTypeId() == ID_UNIT && (ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_OBEYS_TAUNT_DIMINISHING_RETURNS))
        {
    ...
    // Some diminishings applies to mobs too (for example, Stun)
              ...
              && ((targetOwner ? (targetOwner->GetTypeId() == ID_PLAYER) : (GetTypeId() == ID_PLAYER))
                  || (GetTypeId() == ID_UNIT && ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_ALL_DIMINISH)))
...
void Unit::SetLevel(uint8 lvl, bool showLevelChange)
{
    ...
    // group update
    if (GetTypeId() == ID_PLAYER && ToPlayer()->GetGroup())
...
void Unit::SetHealth(uint32 val)
{
    if (getDeathState() == DeathState::JustDied)
        val = 0;
    else if (GetTypeId() == ID_PLAYER && getDeathState() == DeathState::Dead)
        val = 1;
    else
    {
    ...
    if (GetTypeId() == ID_UNIT && !(IsPet() && ToPet()->isControlled() && IsControlledByPlayer()) && (prevHealthPct < 30.0 || HealthBelowPct(30)))
    {
...
void Unit::SetPower(Powers power, uint32 val, bool withPowerUpdate /*= true*/, bool fromRegenerate /* = false */)
{
    ...
    if (withPowerUpdate)
    {
        ...
        data << uint8(power);
        data << uint32(val);
        SendMessageToSet(&data, GetTypeId() == ID_PLAYER);
    }
...
uint32 Unit::GetCreatePowers(Powers power) const
{
    ...
    switch (power)
    {
        ...
        case POWER_RAGE:
            return 1000;
        case POWER_FOCUS:
            return (GetTypeId() == ID_PLAYER || !((Creature const*)this)->IsPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0 : 100);
        case POWER_ENERGY:
            return 100;
        case POWER_HAPPINESS:
            return (GetTypeId() == ID_PLAYER || !((Creature const*)this)->IsPet() || ((Pet const*)this)->getPetType() != HUNTER_PET ? 0 : 1050000);
        case POWER_RUNIC_POWER:
            return 1000;
        case POWER_RUNE:
            return 0;
...
void CharmInfo::InitCharmCreateSpells()
{
    ...
    if (_unit->GetTypeId() == ID_PLAYER)                // charmed players don't have spells
    {
        //InitEmptyActionBar();
        return;
    }
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpellInfo, uint32 damage, SpellInfo const* procAura, int8 procAuraEffectIndex, Spell const* procSpell, DamageInfo* damageInfo, HealInfo* healInfo, uint32 procPhase)
{
    ...
    if (GetTypeId() == ID_PLAYER && ToPlayer()->GetSession()->PlayerLoading())
        return;
            ...
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK:
                        ...
                        if (procSpellInfo && (procSpellInfo->SpellFamilyName || GetTypeId() != ID_PLAYER) &&
...
{
    Unit* owner = GetOwner();
    if (!owner || owner->GetTypeId() != ID_PLAYER)
        return;
...
void Unit::RecalculateObjectScale()
{
    ...
    float scaleMin = GetTypeId() == ID_PLAYER ? 0.1f : 0.01f;
...
void Unit::ClearAllReactives()
{
    ...
    if (getClass() == CLASS_WARRIOR && GetTypeId() == ID_PLAYER)
        ClearComboPoints();
}
...
uint32 Unit::GetCastingTimeForBonus(SpellInfo const* spellProto, DamageEffectType damagetype, uint32 CastingTime) const
{
    ...
    if (CastingTime == 0 && GetTypeId() == ID_UNIT && !IsPet())
        return 3500;
...
{
    if (!normalized || GetTypeId() != ID_PLAYER)
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const*& spellProcEvent, ProcEventInfo const& eventInfo)
{
    ...
    if (EventProcFlag & PROC_FLAG_KILL && GetTypeId() == ID_PLAYER)
    {
        bool allow = false;
    ...
    if (!isVictim && GetTypeId() == ID_PLAYER && !spellProto->HasAttribute(SPELL_ATTR3_NO_PROC_EQUIP_REQUIREMENT))
    {
        Player* player = ToPlayer();
...
void Unit::Kill(Unit* killer, Unit* victim, bool durabilityLoss, WeaponAttackType attackType, SpellInfo const* spellProto, Spell const* spell /*= nullptr*/)
{
    ...
    if (victim->GetTypeId() == ID_PLAYER && victim->getClass() == CLASS_PRIEST && !victim->ToPlayer()->HasPlayerFlag(PLAYER_FLAGS_IS_OUT_OF_BOUNDS))
    {
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if (killer && killer->GetTypeId() == ID_UNIT && killer->IsAIEnabled)
    ...
    // achievement stuff
    if (killer && victim->GetTypeId() == ID_PLAYER)
    {
        if (killer->GetTypeId() == ID_UNIT)
            victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE, killer->GetEntry());
        else if (victim != killer && killer->GetTypeId() == ID_PLAYER)
...
void Unit::SetStunned(bool apply)
{
    ...
    else
    {
        ...
        if (GetTypeId() == ID_UNIT)
        {
            ...
            Unit* owner = GetOwner();
            if (!owner || owner->GetTypeId() != ID_PLAYER || !owner->ToPlayer()->IsMounted())
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    ASSERT(type != CHARM_TYPE_POSSESS || charmer->GetTypeId() == ID_PLAYER);
    ...
    if (GetTypeId() == ID_PLAYER && ToPlayer()->GetTransport())
    {
    ...
    if (GetTypeId() == ID_UNIT)
    {
        ...
        if (charmer->GetTypeId() == ID_PLAYER && charmer->getClass() == CLASS_WARLOCK && ToCreature()->GetCreatureTemplate()->type == CREATURE_TYPE_DEMON)
        {
        ...
        if (charmer->GetTypeId() == ID_PLAYER && ToCreature()->CanFly())
    ...
    if (/*type != CHARM_TYPE_VEHICLE &&*/ !GetCharmInfo())
    {
        ...
        else if (type != CHARM_TYPE_VEHICLE)
        {
            ...
            if (type == CHARM_TYPE_CONVERT && GetTypeId() == ID_UNIT)
    ...
    if (playerCharmer)
    {
        switch (type)
        {
            ...
            case CHARM_TYPE_CHARM:
                if (GetTypeId() == ID_UNIT && charmer->getClass() == CLASS_WARLOCK)
                {
...
void Unit::RemoveCharmedBy(Unit* charmer)
{
    ...
    if (GetTypeId() == ID_UNIT && !HasUnitState(UNIT_STATE_STUNNED))
        RemoveUnitFlag(UNIT_FLAG_STUNNED);
    ...
    ASSERT(type != CHARM_TYPE_POSSESS || charmer->GetTypeId() == ID_PLAYER);
    ASSERT(type != CHARM_TYPE_VEHICLE || (GetTypeId() == ID_UNIT && IsVehicle()));
    ...
    if (GetTypeId() == ID_PLAYER || (GetTypeId() == ID_UNIT && !ToCreature()->IsGuardian()))
        DeleteCharmInfo();
}
...
bool Unit::IsInPartyWith(Unit const* unit) const
{
    ...
    if (u1->GetTypeId() == ID_PLAYER && u2->GetTypeId() == ID_PLAYER)
        return u1->ToPlayer()->IsInSameGroupWith(u2->ToPlayer());
    // Xinef: we assume that npcs with the same faction are in party
    else if (u1->GetTypeId() == ID_UNIT && u2->GetTypeId() == ID_UNIT && !u1->IsControlledByPlayer() && !u2->IsControlledByPlayer())
    ...
    else if ((u2->GetTypeId() == ID_PLAYER && u1->GetTypeId() == ID_UNIT && (u1->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT) && u2->ToPlayer()->GetGroup() && !u2->ToPlayer()->GetGroup()->isRaidGroup()) ||
             (u1->GetTypeId() == ID_PLAYER && u2->GetTypeId() == ID_UNIT && (u2->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT) && u1->ToPlayer()->GetGroup() && !u1->ToPlayer()->GetGroup()->isRaidGroup()))
        return true;
    else
        return false;
}
...
bool Unit::IsInRaidWith(Unit const* unit) const
{
    ...
    // Xinef: we assume that npcs with the same faction are in party
        ...
        return u1->GetFaction() == u2->GetFaction();
    else if ((u2->GetTypeId() == ID_PLAYER && u1->GetTypeId() == ID_UNIT && u1->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT) ||
             (u1->GetTypeId() == ID_PLAYER && u2->GetTypeId() == ID_UNIT && u2->ToCreature()->GetCreatureTemplate()->type_flags & CREATURE_TYPE_FLAG_TREAT_AS_RAID_UNIT))
        return true;
    else
        return false;
}
...
void Unit::ApplyResilience(Unit const* victim, float* crit, int32* damage, bool isCrit, CombatRating type)
{
    ...
    if (victim->IsVehicle() && victim->GetTypeId() != ID_PLAYER)
        return;

    Unit const* target = nullptr;
    if (victim->GetTypeId() == ID_PLAYER)
        target = victim;
    else if (victim->GetTypeId() == ID_UNIT)
    {
...
void Unit::SetPhaseMask(uint32 newPhaseMask, bool update)
{
    ...
    if (IsInWorld())
    {
        ...
        if (GetTypeId() == ID_UNIT || (!ToPlayer()->IsGameMaster() && !ToPlayer()->GetSession()->PlayerLogout()))
        {
    ...
    for (ControlSet::const_iterator itr = m_Controlled.begin(); itr != m_Controlled.end(); )
    {
        Unit* controlled = *itr;
        ++itr;
        if (controlled->GetTypeId() == ID_UNIT)
        {
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        if (spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE) && caster->GetTypeId() == ID_PLAYER && caster->FindMap() && caster->FindMap()->IsBattleArena())
            continue;
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (Player* player = ToPlayer())
    {
        if (vehicle->GetBase()->GetTypeId() == ID_PLAYER && player->IsInCombat())
            return;
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    if (GetTypeId() == ID_UNIT && !CanFly() && vehicleInfo && (vehicleInfo->m_ID == 113 || vehicleInfo->m_ID == 8 || vehicleInfo->m_ID == 290 || vehicleInfo->m_ID == 298))
    {
    ...
    if ((!player || !(player->GetDelayedOperations() & DELAYED_VEHICLE_TELEPORT)) && vehicle->GetBase()->HasUnitTypeMask(UNIT_MASK_MINION))
        if (((Minion*)vehicleBase)->GetOwner() == this)
        {
            ...
            else if (vehicleBase->GetTypeId() == ID_UNIT)
            {
                vehicle->Uninstall();
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    else if (turn)
    {
        ...
        if (GetTypeId() == ID_PLAYER && ToPlayer()->GetFarSightDistance())
...
void Unit::PetSpellFail(SpellInfo const* spellInfo, Unit* target, uint32 result)
{
    ...
    if (!charmInfo || GetTypeId() != ID_UNIT)
        return;
...
int32 Unit::CalculateAOEDamageReduction(int32 damage, uint32 schoolMask, Unit* caster) const
{
    ...
    if (caster && caster->GetTypeId() == ID_UNIT)
...
bool Unit::SetHover(bool enable, bool /*packetOnly = false*/, bool /*updateAnimationTier = true*/)
{
    ...
    else
    {
        ...
        if (hoverHeight && (!isDying() || GetTypeId() != ID_UNIT))
        {
                ...
                if (IsControlledByPlayer() && target != this && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && IsInRaidWith(target))
                {
                    ...
                    if (ft1 && ft2 && !ft1->IsFriendlyTo(*ft2))
                    {
                        ...
                        fieldBuffer << m_uint32Values[index];
                         ...
                         (this->GetTypeId() == ID_PLAYER || this->GetTypeId() == ID_UNIT || this->GetTypeId() == ID_DYNAMICOBJECT))
                {
                    if (index == UNIT_FIELD_BYTES_2)
