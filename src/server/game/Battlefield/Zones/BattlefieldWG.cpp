...
void BattlefieldWG::HandleKill(Player* killer, Unit* victim)
{
    ...
    if (victim->GetTypeId() == ID_PLAYER || IsKeepNpc(victim->GetEntry()) || victim->GetEntry() == NPC_WINTERGRASP_TOWER_CANNON)
    {
        if (victim->GetTypeId() == ID_PLAYER && victim->HasAura(SPELL_LIEUTENANT))
        {
