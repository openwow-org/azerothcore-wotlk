...
public:
    ...
    {
        if (!_introDone && me->IsAlive() && who->GetObjectTypeID() == ID_PLAYER && me->GetExactDist2dSq(who) <= 10000.0f) // 100*100, moveinlineofsight limited to 60yd anyway
        {
            Talk(SAY_ENTER_ZONE);
            _introDone = true;
        }
        ...
        void IsSummonedBy(WorldObject* summoner) override
        {
            ...
            if (Unit* summonerUnit = summoner->ToUnit())
            {
                if (Vehicle* v = summonerUnit->GetVehicle())
                {
                    if (Unit* u = v->GetBase())
                    {
                        if (u->GetEntry() == NPC_BONE_SPIKE && u->GetObjectTypeID() == ID_UNIT)
                        {
