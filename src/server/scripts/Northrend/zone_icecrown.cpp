...
public:
    ...
    struct npc_black_knight_graveyardAI : public ScriptedAI
    {
        ...
        {
            if (who->GetObjectTypeID() != ID_PLAYER || me->GetDistance(who) > 8.0f || who->ToPlayer()->GetQuestStatus(QUEST_BLACK_KNIGHT_CURSE) != QUEST_STATUS_INCOMPLETE)
                return;
...
public:
    ...
    struct npc_guardian_pavilionAI : public ScriptedAI
    {
...

        {
            ...
            if (!who || who->GetObjectTypeID() != ID_PLAYER || !me->IsHostileTo(who) || !me->isInBackInMap(who, 5.0f))
                return;
