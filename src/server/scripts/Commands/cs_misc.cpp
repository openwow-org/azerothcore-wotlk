...
public:
    ...
    static bool HandleDieCommand(ChatHandler* handler)
    {
        ...
        if (target->IsAlive())
        {
            ...
            {
                if (target->GetObjectTypeID() == ID_UNIT && handler->GetSession()->GetSecurity() == SEC_CONSOLE) // pussywizard
                {
    ...
    static bool HandleMovegensCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(LANG_MOVEGENS_LIST, (unit->GetObjectTypeID() == ID_PLAYER ? "Player" : "Creature"), unit->GetGUID().GetCounter());
        ...
        for (uint8 i = 0; i < MAX_MOTION_SLOT; ++i)
        {
            ...
            switch (movementGenerator->GetMovementGeneratorType())
            {
                ...
                {
                    if (unit->GetObjectTypeID() == ID_UNIT)
                    {
