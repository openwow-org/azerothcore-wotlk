...
public:
    ...
    struct npc_pos_collapsing_icicleAI: public NullCreatureAI
    {
        ...
        {
            if (target && spell && target->GetObjectTypeID() == ID_PLAYER && spell->Id == 70827 && pInstance)
...
public:
    ...
    struct npc_pos_leader_secondAI: public NullCreatureAI
    {
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            if ((spell->Id == SPELL_TELEPORT_JAINA || spell->Id == SPELL_TELEPORT_SYLVANAS) && target && target->GetObjectTypeID() == ID_PLAYER)
            {
                float angle = rand_norm() * 2 * M_PI;
