                ...
                switch (eventId)
                {
                    ...
                    case EVENT_BURNING_ADRENALINE:
                    {
                        if (_burningAdrenalineCast < 2) // It's better to use TaskScheduler for this, but zzz
                        {
                            ...
                            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, [&](Unit* u) { return u && !u->IsPet() && u->GetPowerType() == POWER_TYPE_MANA && !u->HasAura(SPELL_BURNING_ADRENALINE) && u != me->GetVictim(); }))
                            {
