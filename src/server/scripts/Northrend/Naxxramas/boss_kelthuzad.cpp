...
public:
    ...
    struct boss_kelthuzadAI : public BossAI
    {
        ...
        {
            if (!me->IsInCombat() && who->GetObjectTypeID() == ID_PLAYER && who->IsAlive() && me->GetDistance(who) <= 50.0f)
                AttackStart(who);
        }
                ...
                case EVENT_DETONATE_MANA:
                    {
                        ...
                        {
                            if (itr->getTarget()->GetObjectTypeID() == ID_PLAYER
...
public:
    ...
    struct boss_kelthuzad_minionAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (who->GetObjectTypeID() != ID_PLAYER && !who->IsPet())
                return;
