...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current, uint32 healthPct /*= 0*/, bool fullMana /*= false*/)
{
    ...
    if (!Create(guid, map, owner->GetPhase(), petInfo->CreatureId, petInfo->PetNumber))
        return false;
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    if (!CreateBaseAtTamed(creature->GetCreatureTemplate(), creature->GetMap(), creature->GetPhase()))
        return false;
...
{
    if (!CreateBaseAtTamed(cinfo, owner->GetMap(), owner->GetPhase()))
        return false;
                    ...
                    case NPC_BLOODWORM:
                        {
                            ...
                            SetReactState(PET_MODE_DEFENSIVE);
                            break;
                        }
