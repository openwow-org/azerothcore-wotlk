...
bool ChaseMovementGenerator<T>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    bool forceDest =
        ...
        (i_target->GetTypeId() == ID_PLAYER && i_target->ToPlayer()->IsGameMaster()) || // for .npc follow
        (owner->CanFly())
...
bool FollowMovementGenerator<T>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    if (PositionOkay(target, owner->IsGuardian() && target->GetTypeId() == ID_PLAYER, targetIsMoving, time_diff))
    {
