...
public:
    ...
    struct boss_halionAI : public BossAI
    {
        ...
        {
            if (victim->GetObjectTypeID() == ID_PLAYER && events.GetNextEventTime(EVENT_KILL_TALK) == 0)
            {
                Talk(SAY_KILL);
...
public:
    ...
    struct boss_twilight_halionAI : public ScriptedAI
    {
        ...
        void KilledUnit(Unit* victim) override
        {
            if (victim->GetObjectTypeID() == ID_PLAYER && _events.GetNextEventTime(EVENT_KILL_TALK) == 0)
            {
                Talk(SAY_KILL);
...
public:
    ...
    class spell_halion_twilight_realm_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*handle*/)
        {
            ...
            if (GetTarget()->GetObjectTypeID() != ID_PLAYER)
                return;
