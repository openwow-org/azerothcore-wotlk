...
AuraObjectType Aura::GetType() const
{
    ...
    return (m_owner->GetObjectTypeID() == ID_DYNAMICOBJECT) ? DYNOBJ_AURA_TYPE : UNIT_AURA_TYPE;
}
...
void Aura::UnregisterSingleTarget()
{
    ...
    if (!caster)
    {
        LOG_INFO("spells", "Aura::UnregisterSingleTarget: (A1) - {}, {}, {}, {}", GetId(), GetOwner()->GetObjectTypeID(), GetOwner()->GetEntry(), GetOwner()->GetName());
    ...
    if (apply)
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_POTION:
                // Alchemy: Mixology
                if (caster && caster->HasAura(53042) && caster->GetObjectTypeID() == ID_PLAYER && !caster->ToPlayer()->GetSession()->PlayerLoading())
                {
...
bool Aura::IsProcTriggeredOnEvent(AuraApplication* aurApp, ProcEventInfo& eventInfo) const
{
    ...
    Unit* target = aurApp->GetTarget();
    if (IsPassive() && target->GetObjectTypeID() == ID_PLAYER && GetSpellInfo()->EquippedItemClass != -1)
    {
