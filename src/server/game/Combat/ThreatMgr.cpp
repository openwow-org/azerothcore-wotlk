...
bool ThreatCalcHelper::isValidProcess(Unit* hatedUnit, Unit* hatingUnit, SpellInfo const* threatSpell)
{
    ...
    // not to GM
    if (hatedUnit->GetObjectTypeID() == ID_PLAYER && hatedUnit->ToPlayer()->IsGameMaster())
        return false;
    ...
    ASSERT(hatingUnit->GetObjectTypeID() == ID_UNIT);
...
void HostileReference::updateOnlineStatus()
{
    ...
    if (isValid()
            && (getTarget()->GetObjectTypeID() != ID_PLAYER || !getTarget()->ToPlayer()->IsGameMaster())
...
void ThreatMgr::_addThreat(Unit* victim, float threat)
{
    ...
    if (!ref) // there was no ref => create a new one
    {
        ...
        if (victim->GetObjectTypeID() == ID_PLAYER && victim->ToPlayer()->IsGameMaster())
