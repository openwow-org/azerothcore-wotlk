...
public:
    ...
    static bool HandleTeleNameNpcIdCommand(ChatHandler* handler, PlayerIdentifier player, Variant<Hyperlink<creature_entry>, uint32> creatureId)
    {
        ...
        CreatureRec const* creatureTemplate = ASSERT_NOTNULL(sObjectMgr->GetCreatureRecord(*creatureId));
    ...
    static bool HandleTeleNameNpcSpawnIdCommand(ChatHandler* handler, PlayerIdentifier player, Variant<Hyperlink<creature>, WOWGUID::LowType> spawnId)
    {
        ...
        CreatureRec const* creatureTemplate = ASSERT_NOTNULL(sObjectMgr->GetCreatureRecord(spawnpoint->id1));
