...
struct ClassCallSelector : public Acore::unary_function<Unit*, bool>
{
    ...
    {
        if (!_me || !target || target->GetObjectTypeID() != ID_PLAYER)
        {
            return false;
        }
            ...
            switch (eventId)
            {
                ...
                case EVENT_CLASSCALL:
                    if (classesPresent.empty())
                    {
                        ...
                        {
                            if (ref->getTarget() && ref->getTarget()->GetObjectTypeID() == ID_PLAYER)
                            {
...
class spell_class_call_polymorph : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        targets.remove_if([&](WorldObject const* target) -> bool
            {
                return target->GetObjectTypeID() != ID_PLAYER || target->ToPlayer()->IsGameMaster() || target->ToPlayer()->HasAura(SPELL_POLYMORPH);
            });
