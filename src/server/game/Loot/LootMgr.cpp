...
bool LootItem::AllowedForPlayer(Player const* player, WOWGUID source) const
{
    ...
    if (!sConditionMgr->IsObjectMeetToConditions(const_cast<Player*>(player), conditions))
    {
        ...
        {
            if ((pProto->Flags & ITEM_FLAG_HIDE_UNUSABLE_RECIPE) || (pProto->Class == ITEM_RECIPE && pProto->Bonding == ITEM_BIND_ON_ACQUIRE && pProto->Spells[1].SpellId != 0))
            {
                return true;
            }
        }
    ...
    if ((pProto->Flags2 & ITEM_FLAG2_FACTION_HORDE) && player->GetTeamId(true) != TEAM_HORDE)
    {
        return false;
    }

    if ((pProto->Flags2 & ITEM_FLAG2_FACTION_ALLIANCE) && player->GetTeamId(true) != TEAM_ALLIANCE)
    {
        return false;
    }
    ...
    if (pProto->Class == ITEM_RECIPE && pProto->Bonding == ITEM_BIND_ON_ACQUIRE && pProto->Spells[1].SpellId != 0 && player->HasSpell(pProto->Spells[1].SpellId))
    {
        return false;
    }
