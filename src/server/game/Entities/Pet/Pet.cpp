...
Pet::Pet(Player* owner, PetType type) : Guardian(nullptr, owner ? owner->GetGUID() : ObjectGuid::Empty, true),
    ...
    m_tempspell(0)
{
    ASSERT(m_owner && m_owner->GetObjectTypeID() == ID_PLAYER);
...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current, uint32 healthPct /*= 0*/)
{
    ...
    if (owner->GetObjectTypeID() == ID_PLAYER && isControlled() && !isTemporarySummoned() && (getPetType() == SUMMON_PET || getPetType() == HUNTER_PET))
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    else                                            // not exist in DB, use some default fake data
    {
        ...
        float factorHealth = owner->GetObjectTypeID() == ID_PLAYER ? std::min(1.0f, cinfo->ModHealth) : cinfo->ModHealth;
        float factorMana = owner->GetObjectTypeID() == ID_PLAYER ? std::min(1.0f, cinfo->ModMana) : cinfo->ModMana;
