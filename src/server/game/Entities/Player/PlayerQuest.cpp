...
void Player::SendPreparedQuest(ObjectGuid guid)
{
    ...
    if (questMenu.GetMenuItemCount() == 1)
    {
        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
        {
            ...
            {
                Object* object = ObjectAccessor::GetObjectByTypeMask(*this, guid, TYPE_UNIT | TYPE_GAMEOBJECT | TYPE_ITEM);
...
void Player::RewardQuest(Quest const* quest, uint32 reward, Object* questGiver, bool announce, bool isLFGReward)
{
    ...
    if (quest->GetRewSpellCast() > 0)
    {
        ...
        if (questGiver->isType(TYPE_UNIT) && !spellInfo->HasEffect(SPELL_EFFECT_LEARN_SPELL) && !spellInfo->HasEffect(SPELL_EFFECT_CREATE_ITEM) && !spellInfo->IsSelfCast())
        {
