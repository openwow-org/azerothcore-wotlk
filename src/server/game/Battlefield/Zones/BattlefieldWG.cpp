...
void BattlefieldWG::HandleKill(Player* killer, Unit* victim)
{
    ...
    if (victim->GetObjectTypeID() == ID_PLAYER || IsKeepNpc(victim->GetEntry()) || victim->GetEntry() == NPC_WINTERGRASP_TOWER_CANNON)
    {
        if (victim->GetObjectTypeID() == ID_PLAYER && victim->HasAura(SPELL_LIEUTENANT))
        {
