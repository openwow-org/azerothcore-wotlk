...
public:
    ...
    struct boss_thorimAI : public ScriptedAI
    {
        ...
        {
            if (who && _isHitAllowed && who->GetPositionZ() > 430 && who->GetObjectTypeID() == ID_PLAYER)
            {
                _isHitAllowed = false;
                DisableThorim(false);
        ...
        {
            if (spellInfo->Id == SPELL_LIGHTNING_CHARGE_DAMAGE && target->GetObjectTypeID() == ID_PLAYER)
...
public:
    ...
    struct boss_thorim_start_npcsAI : public ScriptedAI
    {
        ...
        {
            if (!_playerAttack && who && (who->GetObjectTypeID() == ID_PLAYER || who->GetOwnerGUID().IsPlayer()))
            {
                if (me->GetInstanceScript())
